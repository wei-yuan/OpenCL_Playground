...
float run_equalizeHist(cv::VideoCapture video)
{
    ...
    for (;;)
    {
        ...
        int64_t t1 = (double)cv::getTickCount();
    ...
    */
    ...
    return video.get(cv::CAP_PROP_FRAME_COUNT) / total_timer_eqHist;    
}
}
...
{
    video.set(cv::CAP_PROP_POS_FRAMES, 0);
    double vidoe_frame = video.get(CV_CAP_PROP_FRAME_COUNT);    
    ...
    ////////////////////////////////////////////////////////////////    
    cv::Mat src; // input
    video.read(src);   // 1 frame for init

    int num_of_image_per_batch = 1;

    cv::cvtColor(src, src, CV_BGR2GRAY); // src:: gray scale

    cv::Mat dst = cv::Mat::zeros(src.size(), src.type()),
            lut_1(1, BINS, CV_8UC1), lut_2(1, BINS, CV_8UC1); // output
    std::cout << "dst_1 size: " << dst.size() << std::endl;
    ...
    //                           sizeof(uchar) * dst_1.rows * dst_1.cols * num_of_image_per_batch, dst_1.data, &err);
    // cl_dst_2 = clCreateBuffer(context, CL_MEM_USE_HOST_PTR,
    //                           sizeof(uchar) * dst_1.rows * dst_1.cols * num_of_image_per_batch, dst_2.data, &err);
    if (/*cl_src_1 == 0 || cl_src_2 == 0 ||*/ cl_ghist_1 == 0 || cl_lut_1 == 0 || cl_ghist_2 == 0 || cl_lut_2 == 0 /*|| cl_dst_1 == 0 || cl_dst_2 == 0*/)
    {
        std::cout << "buffer error, err #: " << err << std::endl;
        release_hist_opencl(context, queue_1, queue_2, program_histogram, program_calcLUT, program_LUT, cl_src_1,
                            cl_src_2, cl_ghist_1, cl_lut_1, cl_dst_1, cl_ghist_2, cl_lut_2, cl_dst_2,
                            kernel_calculate_histogram_1, kernel_calcLUT_1, kernel_LUT_1, kernel_calculate_histogram_2,
                            kernel_calcLUT_2, kernel_LUT_2);
    ...
    kernel_calcLUT_2 = clCreateKernel(program_calcLUT, "calcLUT", &err);
    if (err != CL_SUCCESS)
    {
        if (kernel_calcLUT_1 == NULL)
            std::cout << "kernel: Can't load kernel kernel_calcLUT" << std::endl;
    ...
    int64_t end = cv::getTickCount();
    double  init_time = (end - start) / cv::getTickFrequency();    
    ...
    cv::Mat tmphist_1, tmphist_2;
    cv::calcHist( &src, 1, 0, cv::Mat(), tmphist_1, 1, &histSize, &histRange, uniform, accumulate );    
    cv::transpose(tmphist_1, tmphist_1);
    tmphist_1.copyTo(ghist_1(cv::Rect(0, 0, tmphist_1.cols, tmphist_1.rows)));
    end = cv::getTickCount();
    double calcHist_time = (end - start) / cv::getTickFrequency();

    start = cv::getTickCount();
    if (clEnqueueNDRangeKernel(queue_1, kernel_calcLUT_1, 1, 0, globalws_calclut, localws_calclut, 0/*1*/, 0/*&event_even[0]*/,
                               &event_even) != CL_SUCCESS)
    {
    ...
    clFlush(queue_1);
    clWaitForEvents(1, &event_even);
    end = cv::getTickCount();
    double calcLUT_time = (end - start) / cv::getTickFrequency();

    start = cv::getTickCount();
    cv::LUT(src, lut_1, dst);
    end = cv::getTickCount();
    double LUT_time = (end - start) / cv::getTickFrequency();

    std::cout << "calcHist_time: " << calcHist_time << " ,calcLUT_time: " << calcLUT_time << ", LUT_time: " << LUT_time << std::endl;
    // cv::imshow("dst", dst);
    // cv::waitKey(3);
    ...
    //
    ///////////////////////////////////////////////////////////////////////            
    double total_timer_eqHist = 0;    
    for (int i = 1; i < video.get(CV_CAP_PROP_FRAME_COUNT); i++)
    {
        video.read(src);
        ...
        if (i % 2 != 0)
        {
            cv::calcHist( &src, 1, 0, cv::Mat(), tmphist_2, 1, &histSize, &histRange, uniform, accumulate );    
            cv::transpose(tmphist_2, tmphist_2);
            tmphist_2.copyTo(ghist_2(cv::Rect(0, 0, tmphist_2.cols, tmphist_2.rows)));
            ...
            if (clEnqueueNDRangeKernel(queue_2, kernel_calcLUT_2, 1, 0, globalws_calclut, localws_calclut, 1,
                                         &event_even, &event_odd) != CL_SUCCESS)
            {
                std::cout << "Can't enqueue kernel kernel_calcLUT" << std::endl;
                std::cout << "err = " << err << std::endl;
            ...
            clFlush(queue_2);
            clWaitForEvents(1, &event_odd);
            cv::LUT(src, lut_2, dst);                            
            // cv::imshow("dst", dst);
            // cv::waitKey(3);            
        }
        ///////////////////////////////////////////////////////////////////////
        ...
        {            
            ...
            if (clEnqueueNDRangeKernel(queue_1, kernel_calcLUT_1, 1, 0, globalws_calclut, localws_calclut, 1,
                                         &event_odd, &event_even) != CL_SUCCESS)
            {
                std::cout << "Can't enqueue kernel kernel_calcLUT" << std::endl;
                std::cout << "err = " << err << std::endl;
                release_hist_opencl(context, queue_1, queue_2, program_histogram, program_calcLUT, program_LUT,
        ...
        total_timer_eqHist += (end - start) / cv::getTickFrequency();
    }
    ////////////////////////////////
    ...
    double timer_eqHist_finish = (end - start) / cv::getTickFrequency();
    ...
    double re_time = (end - start) / cv::getTickFrequency();

    // double total_frame = ( i + 1 ) * capture.get(CV_CAP_PROP_FRAME_COUNT);
    // output time

    // std::cout << "init_time: " << init_time << std::endl;
    std::cout << "time of of eqHist with clfinish: " << timer_eqHist_finish + total_timer_eqHist << std::endl;
    // std::cout << "time of release resource: " << re_time << std::endl;

    float eqHist_time = timer_eqHist_finish + total_timer_eqHist;
    // std::cout << "eqHist_time: " << eqHist_time << std::endl;

    return video.get(cv::CAP_PROP_FRAME_COUNT) / eqHist_time;    
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        std::cout << "cpu_fps: " << cpu_fps << ", gpu_fps: " << gpu_fps << std::endl;
        // speed up
