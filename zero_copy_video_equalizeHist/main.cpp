...
float run_equalizeHist(cv::VideoCapture video)
{
    ...
    video.read(input);   // 1 frame for init
    ...
    cl_src_2 = clCreateBuffer(context, CL_MEM_USE_HOST_PTR,
                              sizeof(uchar) * src.rows * src.cols * num_of_image_per_batch, src.data, &err);
    ...
    kernel_calculate_histogram_2 = clCreateKernel(program_histogram, "calculate_histogram", &err);
    if (err != CL_SUCCESS)
    {
    ...
    //         std::cout << "kernel: CL_INVALID_KERNEL_NAME" << std::endl;
    // }
    // kernel_LUT_2 = clCreateKernel(program_LUT, "LUT", &err);
    // if (err != CL_SUCCESS)
    // {
    ...
    // // kernel_LUT_2
    // clSetKernelArg(kernel_LUT_2, 0, sizeof(cl_mem), &cl_src_1);
    ...
    if (clEnqueueNDRangeKernel(queue_1, kernel_calcLUT_1, 1, 0, globalws_calclut, localws_calclut, 1, &event_even[0],
                               &event_even[1]) != CL_SUCCESS)
    {
    ...
    // err = clEnqueueNDRangeKernel(queue_1, kernel_LUT_1, 2, 0, globalws_lut, NULL, 1, &event_even[1], &event_even[2]);
    // if (err != CL_SUCCESS)
    // {
    ...
    for (int i = 1; i < video.get(CV_CAP_PROP_FRAME_COUNT); i++)
    {
        ...
        {
            // execute the kernel            
            if (clEnqueueNDRangeKernel(queue_2, kernel_calculate_histogram_2, 1, 0, globalws_hist, localws_hist, 1,
                                         &event_even[1], &event_odd[0]) != CL_SUCCESS)
            {
                std::cout << "Can't enqueue kernel kernel_calculate_histogram" << std::endl;
                std::cout << "err = " << err << std::endl;
            ...
            // std::endl;
            // err = clEnqueueNDRangeKernel(queue_2, kernel_LUT_2, 2, 0, globalws_lut, NULL, 1, &event_odd[1],
            //                              &event_odd[2]);
            // if (err != CL_SUCCESS)
            // {
            //     std::cout << "Can't enqueue kernel kernel_LUT" << std::endl;
        ...
        {
            // execute the kernel            
            if (clEnqueueNDRangeKernel(queue_1, kernel_calculate_histogram_1, 1, 0, globalws_hist, localws_hist, 1,
                                         &event_odd[1], &event_even[0]) != CL_SUCCESS)
            {
                std::cout << "Can't enqueue kernel kernel_calculate_histogram" << std::endl;
                std::cout << "err = " << err << std::endl;
                release_hist_opencl(context, queue_1, queue_2, program_histogram, program_calcLUT, program_LUT,
