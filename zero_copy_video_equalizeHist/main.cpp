...
int main(int argc, char **argv)
{
    ...
    std::string input_file = argv[1] != NULL ? argv[1] : "/home/alex504/img_and_video_data_set/video/1min/720p_1min.mp4";
    ...
    else
    {
        ...
        cl_src = clCreateBuffer(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_ONLY,
                                sizeof(uchar) * src.rows * src.cols, src.data, &err);
        if (cl_src == 0)
        {
        ...
        cl_ghist_1 = clCreateBuffer(context, /*CL_MEM_ALLOC_HOST_PTR |*/ CL_MEM_READ_WRITE,
                                    sizeof(int) * ghist.rows * ghist.cols, NULL, &err);
        if(err != CL_SUCCESS)
            std::cout << "err code: " << err << std::endl;
        cl_lut_1 = clCreateBuffer(context, /*CL_MEM_ALLOC_HOST_PTR |*/ CL_MEM_READ_WRITE,
                                  sizeof(uchar) * lut.rows * lut.cols, NULL/*lut.data*/, &err);
        if(err != CL_SUCCESS)
            std::cout << "err code: " << err << std::endl;
        cl_ghist_2 = clCreateBuffer(context,/*CL_MEM_ALLOC_HOST_PTR |*/ CL_MEM_READ_WRITE,
                                    sizeof(int) * ghist.rows * ghist.cols, NULL, &err);
        if(err != CL_SUCCESS)
            std::cout << "err code: " << err << std::endl;
        cl_lut_2 = clCreateBuffer(context, /*CL_MEM_ALLOC_HOST_PTR |*/ CL_MEM_READ_WRITE,
                                  sizeof(uchar) * lut.rows * lut.cols, NULL/*lut.data*/, &err);        
        if(err != CL_SUCCESS)
            std::cout << "err code: " << err << std::endl;
        ...
        cl_dst_1 = clCreateBuffer(context, /*CL_MEM_ALLOC_HOST_PTR |*/ CL_MEM_WRITE_ONLY,
                                  sizeof(uchar) * dst_1.rows * dst_1.cols, NULL/*dst_1.data*/, NULL);        
        if(err != CL_SUCCESS)
            std::cout << "err code: " << err << std::endl;
        cl_dst_2 = clCreateBuffer(context, /*CL_MEM_ALLOC_HOST_PTR |*/ CL_MEM_WRITE_ONLY,
                                  sizeof(uchar) * dst_1.rows * dst_1.cols, NULL/*dst_2.data*/, NULL);        
        if(err != CL_SUCCESS)
            std::cout << "err code: " << err << std::endl;
        ...
        //
        ///////////////////////////////////////////////////////////////////////
        if (clEnqueueWriteBuffer(data_queue, cl_src, CL_FALSE, 0, sizeof(uchar) * src.rows * src.cols, src.data, 0, 0,
        ...
        // execute the kernel
        if (clEnqueueNDRangeKernel(compute_queue, kernel_calculate_histogram_1, 1, 0, globalws_hist, localws_hist, 1, &write_complete,
        ...
        err = clEnqueueNDRangeKernel(compute_queue, kernel_LUT_1, 2, 0, globalws_lut, NULL, 1, &event_even[1],
                                   &event_even[2]);
        if ( err != CL_SUCCESS)
        {
        ...
        for (int i = 1; i < capture.get(CV_CAP_PROP_FRAME_COUNT); i++)
        {
            ...
            if (i % 2 != 0)
            {
                ...
                if (clEnqueueNDRangeKernel(compute_queue, kernel_calculate_histogram_2, 1, 0, globalws_hist,
                                           localws_hist, 1, &write_complete, &event_odd[0]) != CL_SUCCESS)
                {
                    std::cout << "Can't enqueue kernel kernel_calculate_histogram" << std::endl;
                    std::cout << "err = " << err << std::endl;
                ...
                // std::endl;
                err = clEnqueueNDRangeKernel(compute_queue, kernel_LUT_2, 2, 0, globalws_lut, NULL, 1, &event_odd[1],
                                           &event_odd[2]);
                if ( err != CL_SUCCESS)
                {
                    std::cout << "Can't enqueue kernel kernel_LUT" << std::endl;
            ...
            if (i % 2 == 0)
            {
                ...
                if (clEnqueueNDRangeKernel(compute_queue, kernel_calculate_histogram_1, 1, 0, globalws_hist,
                                           localws_hist, 1, &write_complete, &event_even[0]) != CL_SUCCESS)
                {
                    std::cout << "Can't enqueue kernel kernel_calculate_histogram" << std::endl;
                    std::cout << "err = " << err << std::endl;
                    release_hist_opencl(context, compute_queue, data_queue, program_histogram, program_calcLUT,
