...
float run_equalizeHist(cv::VideoCapture video)
{
    ...
    cv::Mat *srcN[5], *ghistN[5], *lutN[5], *dstN[5];
    int      num_of_image_per_batch = 5;
    int      srcN_basic_mem_size    = src.rows * src.cols * sizeof(uchar);

    uchar *srcN_data = (uchar *)malloc(num_of_image_per_batch * srcN_basic_mem_size);

    cv::Mat lut_1(1, BINS, CV_8UC1), lut_2(1, BINS, CV_8UC1), dst_1 = cv::Mat::zeros(src.size(), src.type()),
                                                              dst_2 = cv::Mat::zeros(src.size(), src.type()); // output
    ...
    int    lutN_basic_mem_size   = lut_1.rows * lut_1.cols * sizeof(uchar);
    uchar *lutN_data             = (uchar *)malloc(num_of_image_per_batch * lutN_basic_mem_size);
    int    dstN_basic_mem_size   = src.rows * src.cols * sizeof(uchar);
    ...
    // size_t localws_copy[1]  = {128};

    // opencl event
    cl_event event_odd[0], event_even[0], write_complete, read_complete, copy_complete;
    ...
    // {
    //     video.read(input);
    //     srcN[i] = new cv::Mat(src.rows, src.cols, CV_8UC1, &srcN_data[i * srcN_basic_mem_size]);
    //     cv::cvtColor(input, *(srcN[i]), CV_BGR2GRAY);
    // }
    
    uchar *dataIn, *dataOut, *startPtr, *ghistPtr, *lutPtr;

    // clSetKernelArg(kernel_copy, 0, sizeof(cl_mem), &cl_src_1);
    // clSetKernelArg(kernel_copy, 1, sizeof(cl_mem), &cl_dst_1);

    num_of_image_per_batch = 5;
    // address for Mat data to write in. dataIn is unchangeable
    dataIn = (uchar *)clEnqueueMapBuffer(queue_1, cl_src_1, CL_TRUE, CL_MAP_WRITE, 0,
                                         sizeof(uchar) * src.rows * src.cols * num_of_image_per_batch, 0, 0,
                                         &read_complete, &err);

    err = clWaitForEvents(1, &read_complete);
    if(err != CL_SUCCESS)
    {
    ...
    cv::Mat tmp;
    startPtr = dataIn;            
    src_offset = src.rows * src.cols * sizeof(uchar);
    ...
    src = cv::Mat(src.rows * num_of_image_per_batch, src.cols, src.type(), dataIn);
    ...
    // err = clEnqueueNDRangeKernel(queue_1, kernel_copy, 1, 0, globalws_copy, 0, 1, &read_complete, &event_even[0]);
    // if (err != CL_SUCCESS)
    // {
    //     std::cout << "Can't enqueue kernel kernel_copy" << std::endl;
    //     std::cout << "err = " << err << std::endl;
    //     release_hist_opencl(context, queue_1, queue_2, program_histogram, program_calcLUT, program_LUT, cl_src_1,
    //                         cl_src_2, cl_ghist_1, cl_lut_1, cl_dst_1, cl_ghist_2, cl_lut_2, cl_dst_2,
    //                         kernel_calculate_histogram_1, kernel_calcLUT_1, kernel_LUT_1, kernel_calculate_histogram_2,
    ...
    dataOut = (uchar *)clEnqueueMapBuffer(queue_1, cl_dst_1, CL_TRUE, CL_MAP_READ, 0,
                                          sizeof(uchar) * src.rows * src.cols, 1,
                                          &event_even[0], &copy_complete, &err);
    if (err != CL_SUCCESS)
    {
    ...
    if(err != CL_SUCCESS)
    {
        std::cout << "clEnqueueMapBuffer cl_dst_1 not success" << std::endl;
    }

    dst = cv::Mat(src.rows, src.cols, src.type(), dataOut);
