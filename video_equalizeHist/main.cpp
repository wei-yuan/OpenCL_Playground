...
void ocl_equalizeHist(cv::Mat& src, cv::Mat& lut, cv::Mat& dst)
{
    ...
    if (queue == NULL)
    {
        ...
        release_hist_opencl(context, queue, program_histogram, program_calcLUT, cl_src, cl_ghist, cl_lut, kernel_calculate_histogram,
                            kernel_calcLUT);
    }
...
int main(int argc, char **argv)
{
    ...
    cv::Mat dst = cv::Mat::zeros(src.size(), src.type()), lut(1, BINS, CV_8UC1);;
    ...
    GPU::ocl_equalizeHist(src, lut, dst);
